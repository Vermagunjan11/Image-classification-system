...
def load_model(model_name: str):
    ...
    try:
        ...
        filename = mapping.get(model_name, model_name)
        model_path = f'saved_models/{filename}_model.h5'
...
def preprocess_image_for_model(image: Image.Image,
    ...
    # Convert image to RGB format to handle RGBA (PNG) or Grayscale
    ...
    if model_type == 'custom':
        ...
        # Keep in [0, 255] range as transfer models have built-in Rescaling layers
...
def main():
    ...
    # Predict Page
        ...
        if st.session_state.uploaded_image is not None:
            ...
            filename = mapping[model_choice]
            model_path = f'saved_models/{filename}_model.h5'
            model_exists = os.path.exists(model_path)
            
            if not model_exists:
                st.warning(f"⚠️ Model file for **{model_choice}** was not found. You need to train it first.")
                if model_choice == "Custom CNN":
                    if st.button("🧠 Train a Quick Demo Model (15s)", use_container_width=True):
                        with st.spinner("Training Custom CNN on a subset of CIFAR-10 (1 epoch)..."):
                            try:
                                import sys
                                # Add 'src' directory to path to resolve imports in train.py
                                ...
                                st.write("Generating dummy training dataset (to avoid slow CIFAR-10 downloads)...")
                                loader = DataLoader()
                                X_sub = np.random.rand(100, 32, 32, 3).astype(np.float32)
                                y_sub = np.random.randint(0, 10, size=(100,))
                                X_sub, y_sub = loader.prepare_data(X_sub, y_sub, num_classes=10)
                                
                                st.write("Building Custom CNN architecture...")
                                cnn_model = create_custom_cnn(input_shape=(32, 32, 3), num_classes=10)
                                
                                st.write("Training model for 1 epoch...")
                                trainer = ModelTrainer('Custom_CNN', cnn_model, batch_size=32, epochs=1)
                                trainer.train(X_sub, y_sub)
                                
                                st.write("Saving model to file...")
                                trainer.save_model(model_path)
                                
                                history_path = f'saved_models/Custom_CNN_history.json'
                                trainer.save_history(history_path)
                                
                                st.success("🎉 Quick Demo Model successfully trained! Reloading...")
                                st.rerun()
                            except Exception as train_err:
                                st.error(f"Error training quick model: {train_err}")
                else:
                    st.info("To use transfer learning models, please run the training script: `python train.py` (requires ~1 hour on CPU).")
            else:
