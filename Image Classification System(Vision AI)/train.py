...
def train_all_models(dataset_name: str = 'cifar10',
    ...
    Args:
        ...
        epochs: Number of epochs
        batch_size: Batch size
        quick_mode: Whether to run in quick mode with subset of data
    ...
    loader = DataLoader()
    if quick_mode:
        logger.info("Quick mode enabled. Generating dummy random dataset for speed and offline testing.")
        X_train = np.random.rand(120, 32, 32, 3).astype(np.float32)
        y_train = np.random.randint(0, 10, size=(120,))
    ...
    # Resize for MobileNetV2 (Keep range in [0, 255] for built-in rescaling layer)
    X_train_mobile = np.array([
        keras.preprocessing.image.smart_resize(img, (224, 224))
        for img in (X_train_split * 255.0)
    ])
    X_val_mobile = np.array([
        keras.preprocessing.image.smart_resize(img, (224, 224))
        for img in (X_val * 255.0)
    ])
    X_test_mobile = np.array([
        keras.preprocessing.image.smart_resize(img, (224, 224))
        for img in (X_test * 255.0)
    ])
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train image classification models on CIFAR-10.")
    parser.add_argument("--quick", action="store_true", help="Train on a tiny subset for 1 epoch to quickly verify pipeline.")
    args = parser.parse_args()
    
    epochs = 1 if args.quick else 20
    
    # Train all models
    results = train_all_models(epochs=epochs, batch_size=32, quick_mode=args.quick)
